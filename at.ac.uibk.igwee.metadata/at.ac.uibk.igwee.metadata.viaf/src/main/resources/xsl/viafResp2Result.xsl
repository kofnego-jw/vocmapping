...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <viafQueryResult>
            ...
            <startRow><xsl:value-of select="//srw:startRecord"/></startRow>
            <maxRows><xsl:value-of select="//srw:maximumRecords"/></maxRows>
            <nextRow>
                <xsl:choose>
                    <xsl:when test="//srw:nextRecordPosition"><xsl:value-of select="//srw:nextRecordPosition"/></xsl:when>
                    <xsl:otherwise>-1</xsl:otherwise>
                </xsl:choose>
            </nextRow>
            <xsl:for-each select="//srw:records/srw:record">
                ...
                <viafVocabulary>
                    ...
                    <linkedSources>
                        <xsl:for-each select="./srw:recordData/ns2:VIAFCluster/ns2:sources/ns2:source">
